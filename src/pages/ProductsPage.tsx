...
const moodTags = ["All Moods", "Subtle Spark", "Effortless Layering", "Late-Night Gold", "City Calm", "Weekend Skin"];
const products = [{
  id: 1,
  name: "Whisper Ring",
  price: 8500,
...
const ProductsPage = () => {
  ...
  const filteredProducts = selectedMood === "All Moods" ? products : products.filter(product => product.mood === selectedMood);
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-wrap justify-center gap-[5px] mb-12 px-0 py-0">
          ...
          animationDelay: '0.2s'
            ...
            {moodTags.map(tag => <button key={tag} onClick={() => setSelectedMood(tag)} className={`mood-tag ${selectedMood === tag ? 'bg-clay text-charcoal' : ''}`}>
                {tag}
              </button>)}
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProducts.map((product, index) => <div key={product.id} className="fade-up" style={{
