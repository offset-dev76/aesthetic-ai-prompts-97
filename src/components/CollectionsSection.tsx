  ...
  return <section className="px-4 py-[80px]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {collections.map((collection, index) => <Link key={collection.name} to={`/products?collection=${collection.tag}`} className={`group hover-lift fade-up ${index === 1 ? 'md:mt-12' : ''}`} style={{
          animationDelay: `${index * 0.2}s`
        }}>
              <div className="relative overflow-hidden rounded-lg mb-4">
                <img src={collection.image} alt={collection.name} className="w-full h-80 object-cover transition-transform duration-500 group-hover:scale-105" />
